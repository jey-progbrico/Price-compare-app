...
import { Search, ArrowRight, ScanBarcode, ChevronRight } from "lucide-react";
...
export default function ProductListClient({ initialProducts }: { initialProducts: any[] }) {
  ...
  const hierarchicalProducts = useMemo(() => {
    ...
    filteredProducts.forEach(p => {
      const cat = p.groupe_produit || "Sans catégorie";
  ...
  return (
    <>
      ...
      <div className="mb-6 px-1 flex justify-between items-center">
        ...
        {query && (
          <button onClick={() => setQuery("")} className="text-xs font-bold text-red-500">Effacer</button>
        )}
      </div>
